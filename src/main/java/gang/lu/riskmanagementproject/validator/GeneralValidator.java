...
public class GeneralValidator {
    ...
    public void validateBatchDbOperateResult(int rows, int expectMinRows) {
        // 1. affect rows cannot be 0
        if (rows <= 0) {
            throw new BizException(HttpStatus.INTERNAL_SERVER_ERROR, COMMON_DATABASE_ERROR);
        }
        // 2. if rows less than expected rows, then must be some id not exist, but the transaction is declared, this validation is only for security
        if (rows != expectMinRows) {
            throw new BizException(HttpStatus.INTERNAL_SERVER_ERROR, COMMON_DATABASE_ERROR);
        }
    }
